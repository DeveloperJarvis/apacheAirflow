...
with DAG('data_approval_dag', start_date=datetime(2023, 10, 1), schedule_interval=None) as dag1:
    start = DummyOperator(task_id='start')
    fetch = PythonOperator(task_id='fetch_data', python_callable=fetch_data)
    preview = PythonOperator(task_id='preview', python_callable=preview)

    start >> fetch >> preview


with DAG('approved_dag', start_date=datetime(2023, 10, 1), schedule_interval=None) as dag2:
...
with DAG('rejected_dag', start_date=datetime(2023, 10, 1), schedule_interval=None) as dag3:
    approve = PythonOperator(task_id='approve_data', python_callable=approve_data, provide_context=True)
